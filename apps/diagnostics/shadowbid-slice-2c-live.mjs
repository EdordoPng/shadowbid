...
const SPECIFICATION = "ShadowBid Slice 2C security review specification v1";
...
async function loadEscrowArtifact() {
  ...
  try {
    ...
    assertCondition(
      typeof artifact.bytecode === "string" && artifact.bytecode !== "0x",
      "contract/deployment failure",
      "ShadowBidEscrow artifact has no deployment bytecode",
    );
    return artifact;
  } catch (error) {
...
async function runFujiPreflight(publicClient, buyerAddress) {
  try {
    const [chainId, avaxBalance, decimals, usdcBalance] = await Promise.all([
...
async function refreshArkivAward() {
  const diagnosticPath = new URL("./arkiv-slice-1d-award-live.mjs", import.meta.url);
  let stdout;

  try {
    ({ stdout } = await execFileAsync(process.execPath, [diagnosticPath.pathname], {
      env: process.env,
      maxBuffer: 1024 * 1024,
    }));
  } catch (error) {
    throw new ScenarioError(
      "Arkiv failure",
      `Slice 1D refresh failed: ${shortMessage(error?.stderr || error)}`,
    );
  }
  ...
  assertCondition(
    ...
    "Arkiv failure",
    "Refreshed Award status or settlement asset is invalid",
  );
  assertCondition(
    award.amount === EXPECTED_AWARD_AMOUNT,
    "Arkiv failure",
    `Refreshed Award amount ${award.amount} is not the canonical Quote amount`,
  );
  assertCondition(
    award.deadline > BigInt(Math.floor(Date.now() / 1_000)) + 600n,
    "Arkiv failure",
    "Refreshed Award deadline is not safely in the future",
  );
...
async function run() {
  ...
  // This read-only preflight completes before the first Fuji state-changing transaction.
  const preflight = await runFujiPreflight(publicClient, configuration.account.address);
  const { output: arkiv, award } = await refreshArkivAward();
  assertCondition(
  ...
  try {
    ...
    deploymentReceipt = await requireSuccessfulReceipt(
      publicClient,
      deploymentTxHash,
      "contract/deployment failure",
      "Deployment",
  ...
  assertCondition(
    deployedCode && deployedCode !== "0x",
    "contract/deployment failure",
    "Deployed escrow has no bytecode",
  );
  ...
  if (allowanceBefore < commitment.amount) {
    try {
      ...
      allowanceAfter = await readUsdc(
        publicClient,
        "allowance",
        [configuration.account.address, escrowAddress],
  ...
  const [buyerBeforeFund, contractBeforeFund] = await Promise.all([
    ...
    readUsdc(publicClient, "balanceOf", [escrowAddress]),
  ]);
  ...
  const [stored, buyerAfterFund, contractAfterFund] = await Promise.all([
    ...
    readUsdc(publicClient, "balanceOf", [escrowAddress], fundingReceipt.blockNumber),
  ]);
  ...
  assertCondition(
    readbackMatches,
    "contract/deployment failure",
    "Escrow readback does not match the Award commitment",
  );
  ...
  assertCondition(
    eventMatches,
    "contract/deployment failure",
    "Funded event does not match the Award commitment",
  );
  assertCondition(
    commitment.procurementId === award.awardId &&
      commitment.amount === award.amount &&
      commitment.deadline === award.deadline &&
      sameHex(commitment.seller, award.seller),
    "application/encoding failure",
    "Cross-layer Award commitment assertions failed",
  );
  assertCondition(
  ...
  console.log("[SHADOWBID SLICE 2C LIVE]");
  ...
  console.log(`deployer: ${configuration.account.address}`);
  ...
  console.log(`deployedBytecode: PRESENT`);
...
try {
  ...
  console.error("[SHADOWBID SLICE 2C LIVE]");
